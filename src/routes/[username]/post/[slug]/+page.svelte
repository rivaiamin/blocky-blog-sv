...
<div class="fixed inset-0 z-60 mx-auto flex max-w-3xl flex-col overflow-hidden bg-white">
	{#if isStories}
		<div class="flex min-h-0 flex-1 flex-col px-4 py-4">
			<PostStoriesView
				...
				onExit={() => goto(resolve(`/${u}`))}
			/>
		</div>
	{:else}
		<div class="min-h-0 flex-1 overflow-y-auto px-4 py-8">
			<article
				class="theme-card overflow-hidden border border-slate-200/80 bg-white/90 backdrop-blur-sm"
			>
				<PostCoverHero
					variant="banner"
