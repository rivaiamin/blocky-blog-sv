...
<div class="mx-auto max-w-4xl">
	<div class="mb-8 flex items-center justify-between">
		<a
			...
			class="text-slate-600 transition-colors hover:text-slate-900">← Dashboard</a
		>
	</div>
	...
	>
		...
		<div>
			...
			<p class="mb-2 text-sm text-slate-500">
				Paste a URL or upload a file (same pipeline as in-editor images).
			</p>
			...
			{/if}
			<label
				class="mt-4 flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 bg-slate-50/80 px-4 py-3"
			>
				<input
					type="checkbox"
					name="coverTextScrim"
				...
				<span>
					...
					<span class="block text-sm text-slate-500"
						>Puts a dark translucent block behind the title and excerpt so they stay readable on
						busy images.</span
					>
				</span>
			</label>
